...
(function () {
    ...
    function initUIModalCompensaciones(pRfc, pObligacionDestino, controlModelo) {
        ...
        var CONTAINER_ID = 0;
        ENTIDAD = FormsBuilder.XMLForm.getCopy().find('control[tipoControl="{0}"]'.format('CuadroDetalleCompensaciones')).find('atributos').find('atributo[valor={0}]'.format(OBLIGACION_DESTINO)).parents().eq(1);
        ...
        if (dlg.length <= 0) {
            ...
            promise.done(function () {
                ...
                dlg.find('#btnTerminar').on("click", function () {
                    ...
                    var propiedades = FormsBuilder.XMLForm.getCopy().find('entidad[id="{0}"]'.format(idEntidadPropiedad)).find('propiedades');
                    ...
                    var getViewModelDetalle = function (propiedades) {
                        ...
                        $.each(rows, function (index, row) {
                            ...
                            $.each(campos, function (index, campo) {
                                ...
                                if (claveInformativa) {
                                    var value = $(campo).val() || '';
                                    var $propiedad = propiedades.find("[claveInformativa='{0}']".format(claveInformativa));
                                    if ($propiedad.attr('tipoDatos') == "Fecha" && !IsNullOrEmpty(value)) {
                                        try {
