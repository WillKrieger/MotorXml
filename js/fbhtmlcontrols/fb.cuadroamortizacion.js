...
(function () {
    ...
    var InfoRegimen = function (name, _formatLabelPEAAE, _formatlabelPEAPAA, id) {
        ...
        this.LabelPEAAE = function (dbId) {
            ...
            var tituloCorto = $(queryPropiedad, formularioXml).find('atributo[nombre="TituloCorto"]').attr("valor");
    ...
    var ModalData = function (ejercicios) {
        ...
        this.adviceNoCalculos = function (data, event) {
            event = event || window.event;
            if (ShowMessageIf(self._perdidas().length === 0 && self._utilidades().length === 0, "No ha capturado alguna utilidad o pérdida. Regrese a la pestaña de '1. Captura' e ingrese algunos datos", event)) {
                preventDefaultEvent(event);
                return;
            }

            if (ShowMessageIf(self.calculos().length === 0 && (self._perdidas().length > 0 && self._utilidades().length > 0), "No ha indicado la aplicación de las pérdidas contra las utilidades.", event)) {
                preventDefaultEvent(event);
                return;
            }
    ...
    var terminarEvent = function (event, dbid) {
        ...
        if (infoRegimenActual) {
            ...
            var message = "Al transferir resultados, se modificarán los campos: <br/>" +
                ...
                "¿Está de acuerdo con la modificación?</br></br>";
