...
(function () {
    ...
    function ControlesGrid(control) {
        ...
        htmlRow.children('.panel').children('.panel-body:last').append('<div style="clear: both;"><button type="button" class="btn btn-primary btn-red btnDelCtrlGridRow" style="float: right;">Eliminar</button>');

        rowNewDiv.find('.ctrlsGrid').append('<b class="leyenda">Para capturar información de este apartado de clic al botón <i>"Agregar"</i></b>');
    ...
    function loadedUIControlesGrid() {
        $('button.btnAddCtrlGridRow').on('click', function (event) {
            ...
            $(this).parent().find('span.help-label').on('click', function () {
                var that = $(this);
            ...
            $(this).parent().find('input[help-text]').on('focus', function () {
                $('#helptext').html($(this).attr('help-text'));
            });
            $(this).parent().find('select[help-text]').on('focus', function () {
                $('#helptext').html($(this).attr('help-text'));
            });
            $(this).parent().find('input[help-text]').blur(function () {
                $('#helptext').html('');
            });
            $(this).parent().find('select[help-text]').blur(function () {
                $('#helptext').html('');
            });
            ...
            $.each(panels, function (k, panel) {
                ...
                // $(panel).children('.panel-heading').attr('id', '{0}de{1}'.format(k+1, panels.length));
            });
                ...
                $.each(viewModels, function (key, ctrlViewModel) {
                    ...
                    objItem[db_id].subscribe(function (newValue) {
                        ...
                        setTimeout(function () {
                            ...
                            // var element = $('#htmlOutput').find('[view-model="{0}"]'.format(db_id));
                            // if (element.hasClass('currency')) {
                            // 	FBUtils.applyFormatCurrencyOnElement(element, true);
                            // }
                        }, 150);
        ...
        $('button.btnDelCtrlGridRow').live('click', function () {
            ...
            if (iconErrors.length > 0) {
                ...
                if (FormsBuilder.ViewModel.getFlujoSecciones()[idEntidadPropiedad].EntroSeccion === true) {
            ...
            if (panels.length === 0) {
                // mainContainer.find("button.btnAddCtrlGridRow:last").addClass('hidden');
