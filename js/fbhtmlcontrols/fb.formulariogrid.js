...
(function () {
    ...
    function loadedUIFormularioGrid() {
        ...
        $('.tabla-formulariogrid tbody tr').live('click', function () {
            ...
            $(this).parents().eq(2).find('input[help-text]').focus(function () {
                $('#helptext').html($(this).attr('help-text'));
            });

            $(this).parents().eq(2).find('select[help-text]').focus(function () {
                $('#helptext').html($(this).attr('help-text'));
            });
            ...
            var xmlCopy = FormsBuilder.XMLForm.getCopy();
            var propiedadesReglas = $(xmlCopy).find('definicionFormulario > definicionReglas > propiedades > propiedad[idEntidadPropiedad="{0}"]'.format($(this).attr('tr-entidad')));
            var listReglas = {};
                            ...
                            if (SAT.Environment.settings('isHydrate') === false) {
                                ...
                                if (validacionGrid !== undefined) {
                                    ...
                                    if (erroresGrid.length > 0) {
                                        ...
                                        columna.html('<i class="icon-warning-sign sat-icon"></i>');

                                        var entidadPadreError = formGridDiv.find('.panel-body').attr('entidadPadre');
                                        if (entidadPadreError !== undefined) {
                                            addErrorMenu(entidadPadreError);
                                        }
                                    } else {
                                        var entidadPadreError = formGridDiv.find('.panel-body').attr('entidadPadre');
                                        if (entidadPadreError !== undefined) {
                                            // removeErrorMenu(entidadPadreError);
                                        }
                ...
                $.each(viewModels, function (key, ctrlViewModel) {
                    ...
                    objItem[db_id].subscribe(function (newValue) {
                        ...
                        if (aplicarRegla && !isDejarSinEfecto && SAT.Environment.settings('isHydrate') === false) {
                            ...
                            setTimeout(function () {
                                if (applyRulesFuncs.length > 10) {
                                    // $('#procesandoReglas').modal('show');
                                    // $('.informacion-declaracion label').html('Procesando....');
                                }
                                if (applyRulesFuncs.length) {
                                    var func = applyRulesFuncs.shift();
                                    func.call();
                                }
                                if (applyRulesFuncs.length <= 0) {
                                    // $('#procesandoReglas').modal('hide');
                                    // $('.informacion-declaracion label').html('');
                                }
                            }, 1);
        ...
        $('button.btnDelFormularioGridRow').on('click', function () {
            ...
            if (iconErrors.length > 0 || iconErrorsChild.length > 0) {
                ...
                var menuItem = $('#sat-sections a[idPanel="{0}"]'.format(FormsBuilder.Parser.getSeccionesUI(entidad)));
                menuItem.parent().find('i').removeClass('icon-warning-sign');
                var idEntidadPropiedad = $('#htmlOutput .panel[id="{0}"]'.format(FormsBuilder.Parser.getSeccionesUI(entidad))).attr('identidadpropiedad');
                if (idEntidadPropiedad !== undefined) {
                    if (String(FormsBuilder.ViewModel.getFlujoSecciones()[idEntidadPropiedad]['EntroSeccion']) === "true") {
                        menuItem.parent().find('i').addClass('icon-signout');
                    }
                }
    ...
    function removeErrors(that, disabled) {
        ...
        objCtrl.find('.sat-val-error').removeClass('sat-val-error');
    ...
    function addErrorMenu(entidad) {
        if (SAT.Environment.settings('isHydrate') === true) return;

        var menuItem = $('#sat-sections a[idPanel="{0}"]'.format(FormsBuilder.Parser.getSeccionesUI(entidad)));
        menuItem.parent().find('i').removeClass('icon-signout');
        menuItem.parent().find('i').removeClass('icon-ok-circle');
        menuItem.parent().find('i').addClass('icon-warning-sign');
        menuItem.parent().find('i').css({ "color": "#CF3928" });
        menuItem.parent().find('a').css({ "color": "#CF3928" });
    }

    function removeErrorMenu(entidad) {
        if (SAT.Environment.settings('isHydrate') === true) return;
